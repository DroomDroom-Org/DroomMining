...
export const useZcash = () => {
  ...
  const [difficultyCurrentTimerange, setDiffcultyCurrentTimerange] = useState('6months');
  const [hashrateCurrentTimerange, setHashrateCurrentTimerange] = useState('6months');
  const [priceCurrentTimerange, setPriceCurrentTimerange] = useState('6months');


  const fetchPriceChart = useCallback(async (timeRange: string = priceCurrentTimerange) => {
    setIsPriceChartLoading(true);
    try {
      const response = await axios.get(getApiUrl(`/zcash/chart/price?timespan=${timeRange}`));
  ...
  const fetchHashrateChart = useCallback(async (timeRange: string = hashrateCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/zcash/chart/hashrate?timespan=${timeRange}`));
  ...
  const fetchDifficultyChart = useCallback(async (timeRange: string = hashrateCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/zcash/chart/difficulty?timespan=${timeRange}`));

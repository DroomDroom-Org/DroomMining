...
export const useDogecoin = () => {
  ...
  const fetchPriceChart = useCallback(async (timeRange: string = priceCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/dogecoin/chart/price?timespan=${timeRange}`));
  ...
  const fetchHashrateChart = useCallback(async (timeRange: string = hashrateCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/dogecoin/chart/hashrate?timespan=${timeRange}`));
  ...
  const fetchDifficultyChart = useCallback(async (timeRange: string = hashrateCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/dogecoin/chart/difficulty?timespan=${timeRange}`));

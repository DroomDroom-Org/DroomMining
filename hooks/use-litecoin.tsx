...
export const useLitecoin = () => {
  ...
  const fetchPriceChart = useCallback(async (timeRange: string = priceCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/litecoin/chart/price?timespan=${timeRange}`));
  ...
  const fetchHashrateChart = useCallback(async (timeRange: string = hashrateCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/litecoin/chart/hashrate?timespan=${timeRange}`));
  ...
  const fetchDifficultyChart = useCallback(async (timeRange: string = hashrateCurrentTimerange) => {
    ...
    try {
      const response = await axios.get(getApiUrl(`/litecoin/chart/difficulty?timespan=${timeRange}`));

...
const MarqueeScroll = ({ tokens }: { tokens: Token[] }) => {
  ...
  const handleClick = (token: Token) => {
    const url = `https://droomdroom.com/price/${token.name
      .toLowerCase()
      .replace(/ /g, "-")}-${token.ticker.toLowerCase()}`;
    window.open(url, "_blank");
  };

  return (
    <div className="w-full bg-background border-b border-border py-1 overflow-hidden">
      <div
        ref={marqueeRef}

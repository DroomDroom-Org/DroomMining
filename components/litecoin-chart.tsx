...
import { chartTimeRanges } from "@/constants/litecoin";

type LitecoinChartProps = {
  title: string;
...
const LitecoinChart: React.FC<LitecoinChartProps> = ({
  title,
  icon,
  xAxisLabel,
  yAxisLabel,
  ...
  const litecoinColor = "hsl(var(--accent-litecoin))";
                ...
                <button
                  ...
                  className={`px-2 py-1.5 text-xs min-w-[36px] text-center font-medium rounded cursor-pointer transition-all duration-200 md:px-3 md:min-w-[32px] md:text-[13px] ${
                    timeRange === range.value
                      ? `bg-litecoin text-white md:font-semibold`
                      : `bg-transparent dark:text-gray-400 text-gray-600 md:font-normal hover:text-litecoin`
                  }`}

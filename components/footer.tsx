...
const Footer = ({ footerData }: FooterProps) => {
  ...
  const socialButton = (social: SocialLink) => {
    // Special handling for Google News which uses a different icon format
    ...
    return (
      <a 
        ...
        className="w-9 h-9 rounded-full flex items-center justify-center hover:opacity-80 transition-all duration-300 hover:scale-110"
      ...
      >
        <Image
          ...
          width={20}
          height={20}
          className={`w-5 h-5 ${isGoogleNews ? '' : 'filter brightness-0 invert'}`}
        />
      </a>
    );
  };

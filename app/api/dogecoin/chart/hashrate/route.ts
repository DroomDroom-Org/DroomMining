...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${BASE_URL}?time=${currentTime}&coin=DOGE&algo=Scrypt`;
    ...
    if (!data?.DOGE || typeof data.DOGE !== 'object') {
      throw new Error('Invalid response format from minerstat API: missing DOGE data');
    }

    const startTimestamp = getStartTimestamp(timespan);
    const interval = getDataInterval(timespan);

    const rawPoints = Object.entries(data.DOGE).map(([ts, values]) => {
      const timestamp = parseInt(ts, 10);
      const difficulty = Array.isArray(values) ? parseFloat(values[1] as string) : NaN;
